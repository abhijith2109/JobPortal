...
class EmployerHomeView(ListView):
    template_name = "emp-home.html"
    model = Jobs
    context_object_name = "jobs"

    def get_context_data(self, **kwargs):
        context = super(EmployerHomeView, self).get_context_data(**kwargs)
        context['allappli'] = Applications.objects.filter(job__posted_by=self.request.user).count()
        context['pend'] = Applications.objects.filter(job__posted_by=self.request.user,status="pending").count()
        context['accappli'] = Applications.objects.filter(job__posted_by=self.request.user,status="accepted").count()

        return context

    def get_queryset(self):
        return Jobs.objects.filter(posted_by=self.request.user).count()
